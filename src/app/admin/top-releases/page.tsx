...
import { ArrowLeft, Search, Loader2, Plus, X, Save, Filter, ChevronLeft, ChevronRight, RefreshCw, BookOpen, Star } from 'lucide-react'
...
export default function TopReleasesManagement() {
  const [searchTerm, setSearchTerm] = useState('')
  const [availableNovels, setAvailableNovels] = useState<Novel[]>([])
  const [filteredNovels, setFilteredNovels] = useState<Novel[]>([])
  const [loading, setLoading] = useState(true)
  const [currentTab, setCurrentTab] = useState('newArrivals')
  const { user } = useAuth()
  ...
  const [newArrivals, setNewArrivals] = useState<Novel[]>([])
  ...
  const [savingEditorsChoice, setSavingEditorsChoice] = useState(false)
  ...
  const fetchTopReleasesContent = async () => {
    try {
      ...
      if (topReleasesDoc.exists()) {
        ...
        const [newArrivalsList, editorsChoiceList] = await Promise.all([
          fetchNovelDetails(data.newArrivals || []),
          fetchNovelDetails(data.editorsChoice || [])
        ])
        
        setNewArrivals(newArrivalsList)
        setEditorsChoice(editorsChoiceList)
      }
    } catch (error) {
      console.error('Error fetching top releases content:', error)
  ...
  const addToCurrentList = (novel: Novel) => {
    if (isNovelInCurrentList(novel.novelId)) {
      toast.error('Novel already in list')
      return
    }

    if (currentTab === 'newArrivals') {
      if (newArrivals.length >= 10) {
        toast.error('Maximum 10 novels allowed for New Arrivals')
        return
      }
      setNewArrivals(prev => [...prev, novel])
    } else {
      if (editorsChoice.length >= 6) {
        toast.error('Maximum 6 novels allowed for Editors\' Choice')
        return
      }
      setEditorsChoice(prev => [...prev, novel])
    }
    
    toast.success(`Added "${novel.title}" to ${currentTab === 'newArrivals' ? 'New Arrivals' : 'Editors\' Choice'}`)
  }

  const removeFromCurrentList = (novelId: string) => {
    if (currentTab === 'newArrivals') {
      setNewArrivals(prev => prev.filter(n => n.novelId !== novelId))
    } else {
      setEditorsChoice(prev => prev.filter(n => n.novelId !== novelId))
    }
    toast.success('Novel removed from list')
  }

  const saveList = async (type: 'newArrivals' | 'editorsChoice') => {
    let setSaving: (value: boolean) => void
    let ids: string[]
    
    switch (type) {
      case 'newArrivals':
        setSaving = setSavingNewArrivals
        ids = newArrivals.map(n => n.novelId)
        break
      case 'editorsChoice':
        setSaving = setSavingEditorsChoice
        ids = editorsChoice.map(n => n.novelId)
        break
    }
    ...
    try {
      ...
      const data = topReleasesDoc.exists() ? topReleasesDoc.data() : {}
      
      // Update only the specific list while preserving others
      await setDoc(topReleasesRef, {
        ...data,
        [type]: ids,
        updatedAt: new Date()
      }, { merge: true })
      
      toast.success(`${type === 'newArrivals' ? 'New Arrivals' : 'Editors\' Choice'} list saved`)
    } catch (error) {
  ...
  const isNovelInCurrentList = (novelId: string) => {
    switch (currentTab) {
      case 'newArrivals':
        return newArrivals.some(n => n.novelId === novelId)
      case 'editorsChoice':
        return editorsChoice.some(n => n.novelId === novelId)
      default:
        return false
    }
  }
  ...
  const getCurrentSaving = () => {
    switch (currentTab) {
      case 'newArrivals':
        return savingNewArrivals
      case 'editorsChoice':
        return savingEditorsChoice
      default:
        return false
    }
  }
  ...
  const handleSave = () => {
    if (currentTab === 'newArrivals') {
      saveList('newArrivals')
    } else {
      saveList('editorsChoice')
    }
  }
                      ...
                      <TableBody>
                        {paginatedNovels.map((novel, index) => (
                          ...
                          >
                            ...
                            <TableCell>
                              <Button
                                onClick={() => addToCurrentList(novel)}
                                disabled={isNovelInCurrentList(novel.novelId)}
                                size="sm"
                                className={`bg-gradient-to-r transition-all duration-300 shadow-md ${
                                  isNovelInCurrentList(novel.novelId)
            ...
            <div className="bg-gradient-to-br from-[#232120] to-[#2A2827] rounded-lg shadow-lg border border-[#333] overflow-hidden">
              <div className="p-6 border-b border-[#333]">
                ...
                <Tabs value={currentTab} onValueChange={setCurrentTab}>
                  <TabsList className="bg-gradient-to-r from-[#2A2827] to-[#333] border border-[#444] p-1">
                    <TabsTrigger 
                      value="newArrivals" 
                      className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#F1592A] data-[state=active]:to-[#FF7B4D] data-[state=active]:text-white text-gray-300 transition-all duration-300"
                    >
                      New Arrivals ({newArrivals.length}/10)
                    </TabsTrigger>
                    <TabsTrigger 
                      value="editorsChoice" 
                      className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#F1592A] data-[state=active]:to-[#FF7B4D] data-[state=active]:text-white text-gray-300 transition-all duration-300"
                    >
                      Editors' Choice ({editorsChoice.length}/6)
                    </TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
              <div className="p-6">
                <div className="space-y-4 max-h-[500px] overflow-y-auto">
                  {getCurrentList().map((novel, index) => (
                    <div 
                      key={novel.novelId} 
                    ...
                    >
                      ...
                      <Button
                        onClick={() => removeFromCurrentList(novel.novelId)}
                        variant="ghost"
                        size="sm"
                    ...
                    <div className="text-center py-12 border-2 border-dashed border-[#444] rounded-lg">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-400 text-sm mt-1">
                          Add novels to {currentTab === 'newArrivals' ? 'New Arrivals' : 'Editors\' Choice'} from the list
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <Button
                  onClick={handleSave}
                  disabled={getCurrentSaving() || getCurrentList().length === 0}
                ...
                >
                  {getCurrentSaving() ? (
                    ...
                    <>
                      ...
                      Save {currentTab === 'newArrivals' ? 'New Arrivals' : 'Editors\' Choice'}
                    </>
                  )}
                </Button>
              </div>
